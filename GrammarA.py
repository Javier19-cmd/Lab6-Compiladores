...
def crear_automataLR(gramma):
    ...
    for state in delta:
        ...
        if F_identificator in state:
            # Colocando en la action el estado como estado de aceptación.
            action[state]["$"] = "acceptance"

        for symbol in sigma:
            transitions = delta[state][symbol]
            if transitions:
        ...
        for item in state:
            item_str = str(item)
            if F_identificator in item_str and item[-1] == ".":
                prod_index = dot_grammar.index(item[:-1])
                prod = dot_grammar[prod_index]
                prod_left = prod[0]
                follow = siguientes[prod_left]
                for symbol in follow:
                    # Reducción (Reduce)
                    if symbol not in action[state]:
                        action[state][symbol] = ("reduce", prod_index)
                    else:
                        # Si hay una reducción y un desplazamiento para el mismo símbolo,
                        # se debe resolver la conflictividad.
                        existing_action = action[state][symbol]
                        if existing_action[0] == "shift":
                            # Resolución de conflicto: Shift-Reduce
                            print("¡Conflicto Shift-Reduce en el estado", state, "con símbolo", symbol, "!")
                            # Aquí puedes agregar la lógica para resolver el conflicto según tus necesidades.
                            # Puedes elegir entre priorizar el Shift o el Reduce.
                            # Por ejemplo, puedes priorizar el Reduce:
                            action[state][symbol] = ("reduce", prod_index)
